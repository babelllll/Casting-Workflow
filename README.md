...
本项目由 qcbigma 在 LinuxDo 社区以「开源推广」形式发布，项目完整开源，并在原帖中说明了设计动机：在短篇小说生成里，尝试在原创性和反检测之间找一个工程化平衡点。

作者最初考虑过更重的相似度方案：MinHash、Winnowing、N-gram 碰撞检测、动态 N 值调整、句式模板聚类、主题词 TF-IDF 余弦相似度等。但这类方案实现成本高、项目复杂度也高；后续测试发现，用更轻量的 `jieba` 分词和类型指纹蒸馏，也可以支撑一个可运行的简化版本。

项目围绕一个关键词展开：**公约数**。

在数学里，公约数是能同时整除几个整数的数。放到文体学和网文工程里，它是一个比喻：从多篇同类型小说中，提取它们共同拥有、反复出现、足够稳定的底层风格基因，同时剔除各自独有的具体情节、人物、道具、专名和表达。

可以把它理解成麻辣香锅的例子：如果你喜欢 5 个不同品牌的麻辣锅底，不是去复制某一家秘方，而是抽出它们共同的「辣、麻、油」这几个类型感，再用这些公约数做一盘新的菜。对应到本项目，就是：

> 指纹互消取交集，让 AI 学的是「类型感」，不是照着某一篇抄作业。

因此，本项目不是单篇仿写器，而是「多篇同类语料 → 指纹互消 → 类型公约数 → 生成提示词」的短篇生产管道。语料质量很关键：必须尽量使用同类短篇，混入题材、风格、平台气质差异很大的文本，会直接稀释公约数。

作者原帖提供了两个补充包：

- `prompt.zip`：缺失的 9 个阶段提示词，解压后放入 `prompt/`。
- `14_现代言情.7z`：现代言情短篇测试语料，解压后放入 `corpus/14_现代言情/`。

本仓库已按上述方式补入：`prompt/` 现在包含完整 9 个提示词文件，`corpus/14_现代言情/` 包含 88 篇 `.txt` 测试语料。

自然语言使用示例：

```text
帮我写一篇 10000 字的重生复仇番茄爽文，执行 12 工作流。
```

当前已知短板：情绪词库仍偏少，后续可以继续扩充，以提升问题项识别和自动修正能力。
...
把同类型小说 `.txt` 放入对应分类目录。目前保留 9 个标准分类，并补入作者原帖的 `14_现代言情` 测试分类：
...
将 `output/fusion_context.txt` 的全部内容**原封不动**复制给 LLM（Claude/GPT/DeepSeek 均可），LLM 据此生成一篇约 10000 字的原创短篇小说。
